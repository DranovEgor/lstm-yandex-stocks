...
def predict_future(model, scaler, initial_sequence, days_to_predict=10):
    ...
    with torch.no_grad():
        for _ in range(days_to_predict):
            ...
            predicted_value = scaler.inverse_transform(prediction.numpy().reshape(-1, 1))
...
def plot_predictions(actual_prices, predicted_prices, sequence_length):
    ...
    pred_dates = range(len(actual_prices), len(actual_prices) + len(predicted_prices) + 1)
    plt.plot(pred_dates, [last_actual] + list(predicted_prices), 
